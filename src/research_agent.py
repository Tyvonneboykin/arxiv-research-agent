...
class ResearchAgent:
    ...
    async def initialize_components(self):
        ...
        try:
            ...
            cache_config = self.config.get_performance_config().cache
    ...
    async def fetch_and_analyze_papers(self) -> tuple[List[ArxivPaper], List[PaperAnalysis]]:
        ...
        # Filter out already analyzed papers using cache
        new_papers = self.cache_manager.filter_new_papers(filtered_papers[:10])  # Limit to top 10 for cost control
        
        if not new_papers:
            # All papers were already analyzed, get cached results
            self.logger.info("🏃‍♂️ All papers already analyzed, retrieving from cache")
            paper_ids = [paper.id for paper in filtered_papers[:10]]
        ...
        for paper, analysis in zip(new_papers, new_analyses):
            self.cache_manager.cache_analysis(paper, analysis)
        
        # Combine with cached analyses if we have a mix
        if len(new_papers) < len(filtered_papers[:10]):
            cached_paper_ids = [p.id for p in filtered_papers[:10] if p not in new_papers]
            cached_analyses = self.cache_manager.get_cached_analyses(cached_paper_ids)
...
async def main():
    ...
    parser.add_argument("--test-run", action="store_true", help="Run a test digest generation")
    
    args = parser.parse_args()
    
    # Print startup info
    print("🤖 ArXiv Research Agent Starting...")
    ...
    try:
        ...
        print(f"✅ Claude Code SDK version: {claude_code_sdk.__version__}")
    except Exception as e:
    ...
    if args.test_run:
        # Run a test digest generation
        print("🧪 Running test digest generation...")
        try:
