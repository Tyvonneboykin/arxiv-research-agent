#!/usr/bin/env python3
"""
Direct Anthropic API Paper Analyzer - Fallback Implementation
=============================================================

Uses direct Anthropic API without Claude Code CLI dependency.
Provides identical analysis functionality with pure Python implementation.
"""
...
@dataclass
class PaperAnalysis:
    """Results of Claude's paper analysis - identical to original"""
    paper_id: str
...
class DirectClaudeAnalyzer:
    """Direct Anthropic API analyzer - no CLI dependency"""
    ...
    def __init__(self, working_dir: str = None):
        ...
        self.logger.info("✅ Direct Anthropic API client initialized")
        ...
        # Analysis templates (identical to original)
...
"""
    ...
    async def analyze_paper(self, paper: ArxivPaper, 
        ...
        """Analyze a single paper using direct Anthropic API"""
        try:
            # Prepare the prompt
            ...
            self.logger.info(f"Analyzing paper with direct API: {paper.title[:50]}...")
            ...
            if not response_text:
                self.logger.error(f"Empty response from API for paper {paper.id}")
                return None
            ...
            if json_start == -1 or json_end == 0:
                self.logger.error(f"No JSON found in API response for paper {paper.id}")
                return None
            ...
            self.logger.info(f"✅ Successfully analyzed paper {paper.id} with direct API")
            return analysis
            ...
            self.logger.error(f"Error analyzing paper {paper.id} with direct API: {e}")
            return None
    ...
    async def analyze_paper_batch(self, papers: List[ArxivPaper], 
        ...
        # Limit concurrent requests for API rate limiting
    ...
    async def identify_trends(self, analyses: List[PaperAnalysis]) -> Dict[str, Any]:
        ...
        try:
            ...
            trend_analysis = message.content[0].text if message.content else ""
    ...
    def rank_papers(self, analyses: List[PaperAnalysis], 
                   criteria: str = "overall") -> List[PaperAnalysis]:
...
"""
        ...
        try:
            ...
            return message.content[0].text if message.content else "Error generating insights"
...
# Example usage
async def main():
    """Example usage of DirectClaudeAnalyzer"""
    ...
    async with ArxivFetcher() as fetcher:
        papers = await fetcher.fetch_ai_papers(days_back=1, max_results=3)
